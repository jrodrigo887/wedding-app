...
export const usePhotosStore = defineStore('photos', () => {
  ...
  const fetchGuestPhotoCount = async (): Promise<void> => {
    ...
    try {
      const mediaCount = await createPhotoRepository().getGuestMediaCount(
        currentGuestCode.value
      );
  ...
  const uploadPhoto = async (file: File, caption?: string): Promise<boolean> => {
    ...
    try {
      const result = await createPhotoRepository().uploadPhoto({
  ...
  const uploadMedia = async (
    ...
    try {
      ...
      if (mediaType === 'video') {
        result = await createPhotoRepository().uploadVideo({
          codigo_convidado: currentGuestCode.value,
          nome_convidado: currentGuestName.value,
          file,
          caption,
        ...
        result = await createPhotoRepository().uploadPhoto({
          codigo_convidado: currentGuestCode.value,
          nome_convidado: currentGuestName.value,
          file,
          caption,
